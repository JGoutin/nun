...
class Tsk:
    """Task"""
    __slots__ = ('_res_names', '_action', '_arguments', '_ui', '_debug',
                 '_tsk_id', '_force')

    def __init__(self, res_names, action, debug=False, force=False,
                 **arguments):
        self._debug = debug
        self._force = force
    ...
    def apply(self):
        ...
        if action in ('update', 'remove'):
            ...
            resources = (
                Res(tsk_id=tsk_id, res_id=row['id'], name=row['name'],
                    action=row['action'], arguments=row['arguments'])
                for glob in self._res_names for row in DB.get_res_by_glob(glob))

        else:
            # New resources
            arguments = self._arguments
            resources = (Res(tsk_id=tsk_id, name=name, action=action,
                             arguments=arguments) for name in self._res_names)
