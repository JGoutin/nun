...
def _run_command():
    ...
    parser = ArgumentParser(
        prog=APP_NAME,
        description="A package manager to install from Git repositories and platforms.",
    )
    ...
    parser.add_argument(
        "--debug",
        action="store_true",
        help="If True, show full error traceback and stop on " "first error.",
    )
    ...
    action.add_argument(
        "--force", "-f", help="Always replace destination.", action="store_true"
    )
    ...
    action.add_argument(
        "--trusted",
        help="Allow extraction of files outside of the " "output directory.",
        action="store_true",
    )
    action.add_argument(
        "--strip_components",
        type=int,
        default=0,
        help="strip NUMBER leading components from file " "path on extraction",
    )
    action.add_argument(
        "--force", "-f", help="Always replace destination.", action="store_true"
    )
    ...
    action = sub_parsers.add_parser("update", help=description, description=description)
    ...
    action = sub_parsers.add_parser("remove", help=description, description=description)
    action.add_argument("resources", nargs="*", help="Resources.", default="*")

    # Parser: "nun info"
    description = "Information about package."
    action = sub_parsers.add_parser("info", help=description, description=description)
    action.add_argument("resources", nargs="*", help="Resources.")

    # Parser: "nun list"
    description = "List packages."
    action = sub_parsers.add_parser("list", help=description, description=description)
